...
class UserOnHoldPage extends StatefulWidget {
  @override
  _UserOnHoldPagePageState createState() => _UserOnHoldPagePageState();
}

class _UserOnHoldPagePageState extends State<UserOnHoldPage> {
  ...
  void initState() {
    ...
    if (_myAnimeListApiCall.list.length >= 0) {
      onHoldList =
          _myAnimeListApiCall.onHoldListeyiDondur(_myAnimeListApiCall.list);
      // print(_userAcc.numItemsOnHold);
    }
  }

  @override
  Widget build(BuildContext context) {
    return _userAcc.numItemsOnHold == null
        ? RefreshIndicator(
            onRefresh: () {
              return Future.delayed(Duration(seconds: 3));
            },
            child: Center(
              child: Text("Henüz bir anime onholdda değil"),
            ),
          )
        : _myAnimeListApiCall.list.length <= 0
            ? RefreshIndicator(
                    ...
                    setState(() {
                      _myAnimeListApiCall
                        ...
                        if (this.mounted) {
                          setState(() {
                            onHoldList =
                                _myAnimeListApiCall.onHoldListeyiDondur(value);
                          });
                        }
                      });
                    });
                ...
                child: Container(
                  child: Center(
                      child: FutureBuilder(
                    future: onHoldList.length <= 0
                        ? _myAnimeListApiCall.userListGetir(_userAcc.numItems)
                        : null,
                    builder: (BuildContext context, AsyncSnapshot snapshot) {
                      if (snapshot.data == null) {
                      } else {
                        onHoldList = _myAnimeListApiCall
                            .onHoldListeyiDondur(snapshot.data);
                        // print(onHoldList.length.toString() +
                        //     " datamızın uzunluğu");
                      }
                      return onHoldList.length <= 0
                          ? Center(
            ...
            : RefreshIndicator(
                ...
                child: Center(
                  child: buildOnHoldPageView(),
                ),
              );
  }

  ListView buildOnHoldPageView() {
    return ListView.builder(
        itemCount: onHoldList.length,
        itemBuilder: (BuildContext context, int index) {
          var anime = onHoldList[index];
          return Column(
            children: [
              InkWell(
                onTap: () {
